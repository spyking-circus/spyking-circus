...
def compute_rho(data, update=None, mratio=0.01):
    ...
    if update is None:
        ...
        sorted_idx = numpy.argsort(dist, axis=1) # sorting each row in ascending order
        dist_sorted = numpy.take_along_axis(dist, sorted_idx, axis=1)
        ...
        sorted_idx = numpy.argsort(all_dist, axis=1) # sorting each row in ascending order
        dist_sorted = numpy.take_along_axis(all_dist, sorted_idx, axis=1)
        rho = numpy.mean(dist_sorted[:, 1:nb_selec+1], axis=1) # density computation
        sdist = dist_sorted[:, 1:nb_selec+1]
