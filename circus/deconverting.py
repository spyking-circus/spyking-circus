...
from circus.shared.utils import logging, comm
...
def main(params, nb_cpu, nb_gpu, use_gpu, extension):
    ...
    if not os.path.isdir(input_path):
        print_and_log(["Can't find {}".format(input_path)], 'error', logger)
        sys.exit(0)
    ...
    and os.path.isfile(result_path):
        print_and_log(["Phy results already imported."], 'error', logger)
        sys.exit(0)
    ...
    or os.path.isfile(result_path):
        print_and_log(["Phy results partially imported."], 'error', logger)
        sys.exit(0)
    ...
    if comm.rank == 0:
        ...
        # TODO add support for [fitting] collect_all=True.
        ...
        for k, index in enumerate(to_keep):
            maxoverlaps[k, :] = overlaps[index, to_keep] * shape[0] * shape[1]
