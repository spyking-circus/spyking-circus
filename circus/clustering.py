            ...
            if elt_count < loop_nb_elts:
                ...
                if matched_filter:
                    ...
                    if sign_peaks in ['positive', 'both']:
                        ...
                        for i in xrange(N_e):
                            peaktimes = scipy.signal.find_peaks(
                                filter_chunk[:, i], height=matched_tresholds_pos[i],
                    ...
                    if sign_peaks in ['negative', 'both']:
                        ...
                        for i in xrange(N_e):
                            peaktimes = scipy.signal.find_peaks(
                                filter_chunk[:, i], height=matched_tresholds_neg[i],
                                width=spike_width, distance=dist_peaks, wlen=N_t
                            )[0]
                            peaktimes = peaktimes.astype(numpy.uint32)
                            all_peaktimes.append(peaktimes)
                                            ...
                                            if max_test:
                                                if gpass == 0:
                                                    ...
                                                    if to_accept:
                                                        ...
                                                        result['data_%s_' % loc_peak + str(elec)].append(sub_mat)# = numpy.vstack((result['data_%s_' % loc_peak + str(elec)], sub_mat))
                                                else:
                                                    ...
                                                    to_accept = True
                                                    result['tmp_%s_' % loc_peak + str(elec)].append(sub_mat)# = numpy.vstack((result['tmp_%s_' % loc_peak + str(elec)], sub_mat))
